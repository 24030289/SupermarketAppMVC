...
app.get("/shopping", checkAuthenticated, (req, res) => {
  ...
  Product.searchProducts(search, (err, products) => {
    ...
    res.render("shopping", { products, user: req.session.user, search, sort });
  });
});
...
app.get("/checkout/finalize", checkAuthenticated, CheckoutController.finalizeAfterPayment);
...
app.get("/sse/payment-status/:txnRetrievalRef", checkAuthenticated, NetsController.paymentStatusSSE);

// NETS result pages (optional, if you want)
app.get("/nets-qr/fail", checkAuthenticated, (req, res) => {
  res.render("netsTxnFailStatus", { message: "NETS Payment Failed" });
});
app.get("/nets-qr/success", checkAuthenticated, (req, res) => {
  // You can skip this page and go straight to finalize
  // but if your netsQr.ejs redirects here, we can forward to finalize.
  const txnRetrievalRef = req.query.txn_retrieval_ref;
  return res.redirect(`/checkout/finalize?method=nets&txnRetrievalRef=${txnRetrievalRef}`);
});

// =========================
// PayPal API endpoints (CA2)
// =========================
